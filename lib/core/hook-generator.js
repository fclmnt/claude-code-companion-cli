...
async function sendNotificationRequest(operation) {
  ...
  // All notifications now require user interaction - no auto-approval
  ...
  const requestData = {
    ...
    notification_type: 'blocking', // All notifications require user interaction
    context: {
  ...
  try {
    ...
    // All operations now wait for user interaction - no auto-approval
    console.log(\`📱 Sent to iPhone: \${operation} (risk: \${riskLevel})\`);
...
async function main() {
  try {
    ...
    // Debug: Log raw input to understand Claude Code format
    if (hookInput) {
      console.log('DEBUG: Raw hook input received:', JSON.stringify(hookInput, null, 2));
    }
    
    let operation;
    if (hookInput) {
      // Try different possible fields where the prompt might be
      operation = hookInput.message || 
                 hookInput.prompt || 
                 hookInput.tool_input?.content ||
                 hookInput.tool_input?.action_description ||
                 hookInput.description ||
                 JSON.stringify(hookInput);
      console.log(\`Processing hook input: \${operation}\`);
    } else {
