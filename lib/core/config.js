...
class Config {
  ...
  getConfig() {
    ...
    // Determine server URL based on sandbox mode
    const defaultServerUrl = fileConfig.sandbox 
    ...
    this._config = {
      ...
      serverUrl: process.env.CC_NOTIFICATIONS_SERVER || fileConfig.serverUrl || defaultServerUrl,
      sandbox: fileConfig.sandbox || false,
  ...
  showConfig() {
    ...
    console.log(`  Environment: ${chalk.cyan(config.sandbox ? 'Sandbox (Develop)' : 'Production')}`);
