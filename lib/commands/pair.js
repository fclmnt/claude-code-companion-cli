...
async function pairCommand(pairingCode, options) {
  console.log(chalk.blue('📱 Pairing with iPhone...\n'));
  ...
  try {
    ...
    spinner.start(`Connecting to server: ${options.server}`);
    ...
    console.log(`2. Test the connection: ${chalk.cyan('ccnotify status')}`);
    console.log(`3. Send a test notification: ${chalk.cyan('ccnotify test')}`);
    console.log('4. Run Claude Code commands - risky operations will trigger iPhone notifications!');
